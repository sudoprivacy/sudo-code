...
// ---------------------------------------------------------------------------
// Import credentials from upstream CLI keychain
// ---------------------------------------------------------------------------
...
pub fn import_claude_code_credentials() -> Result<OAuthTokenSet, String> {
    ...
    let payload: CcKeychainPayload = serde_json::from_str(&json)
        .map_err(|e| format!("failed to parse upstream keychain data: {e}"))?;
...
/// Reads the password from the upstream CLI's legacy keychain entry using the
