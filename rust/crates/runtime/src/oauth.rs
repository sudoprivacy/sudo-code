...
// ---------------------------------------------------------------------------
// Import credentials from the upstream CLI keychain
// ---------------------------------------------------------------------------

const CC_KEYRING_SERVICE: &str = "Claude Code-credentials"; // kept for keychain backward compat

/// JSON shape stored by the upstream CLI in macOS Keychain.
...
/// Reads the upstream CLI's OAuth token from the OS keychain and copies it into
...
pub fn import_claude_code_credentials() -> Result<OAuthTokenSet, String> {
    ...
    let payload: CcKeychainPayload = serde_json::from_str(&json)
        .map_err(|e| format!("failed to parse credential keychain data: {e}"))?;
    let cc = payload.claude_ai_oauth.ok_or_else(|| {
        "Keychain entry has no OAuth token. Run `claude` first to authenticate."
            .to_string()
    })?;
...
/// Reads the password from the legacy keychain entry using the macOS `security`
/// CLI.  The `keyring` crate cannot access legacy keychain entries (it uses the
/// newer Data Protection Keychain API), so we shell out.
fn read_cc_keychain_password() -> Result<String, String> {
    ...
    if !output.status.success() {
        return Err(
            "no credentials found in keychain. Run `claude` first to authenticate."
                .to_string(),
        );
    }
