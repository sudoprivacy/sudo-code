    ...
    T: ToolExecutor,
{
    ...
    /// Like [`run_turn`](Self::run_turn) but accepts pre-built content blocks
    /// (e.g. text + image) instead of a plain string.  The first `Text` block
    /// is used for telemetry labelling.
    #[allow(clippy::too_many_lines)]
